...
isnr = 30 ;%对不同的图片，同一个isnr值对应不同的sigma，这不符合常理，因此弃用。
noise_level = 0.07; 
...
options.rho = 100;  % 在混合算法中没有用到

%step size for the proximal gradient update 需要小于lipschitz continuous gradient
...
%     sigma = norm(im)/sqrt(Nx)*10^(-isnr/20);
   ...
   %  [xsol, rsnr, niter] = admm_BM3D(y_noise,epsilon,Phi_t,Phi,Psi,Psit,options,im,noise_level,validationname{i}(1:(length(2)-5)));
    [xsol, rsnr, niter] = admm_hybird(y_noise,epsilon,Phi_t,Phi,Psi,Psit,options,net.trainedNet,im,validationname{i}(1:(length(2)-4)));
   % [xsol, rsnr, niter] = admm_hybird_convergence(y_noise,epsilon,Phi_t,Phi,Psi,Psit,options,net.trainedNet,im);
   ...
   % write
 %  name=['./project/result/reconstruction_images/hybird/BM3D',validationname{i}(1:(length(2)-4)),'_',num2str(rsnr),'_',num2str(ssimval),'.png'];
   name=['./project/result/figure/hybird/xsol',validationname{i}(1:(length(2)-4)),'_',num2str(rsnr),'_',num2str(ssimval),'.png'];
   imwrite(xsol,name);
   result_matrix(1,i) = i ;
...
save(['./project/result/figure/hybird/xsol/reconstruction.mat'],'result_matrix','options');
%save(['./project/result/reconstruction_images/hybird/BM3D.mat'],'result_matrix','options');
...
Excel_name = ['./project/result/figure/hybird/xsol/reconstruction.csv'];
%Excel_name = ['./project/result/reconstruction_images/prox/prox.csv'];
sheet_name = 'sheet1';
